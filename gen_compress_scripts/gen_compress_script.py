...
BASE_PATH = '/oracle/home/dbscripts/compress/compress_scripts/'
LOG_FILE_BASE = '/oracle/home/dbscripts/compress/compress_logs/'
...
class GenHwmScript(object):
    ...
    def first(self):
        ...
        script_name =  dt1 + '_compress_' + self.tabname + '.sql'
        row1 = 'set timing on\n'
        row2 = 'set time on\n'
        row3 = 'set echo on\n'
        ...
        row7 = 'define fil=/oracle/home/dbscripts/compress/compress_logs' + dt2 + '/' + dt1 + '-Table_compress_' + self.tabname + '.log\n'
        row8 = 'prompt Table_Compress Spooling to &fil\n'
        row9 = 'spool &fil\n'
        row10 = 'prompt ' + dt1 + '_compress_' + self.tabname + ' Table_Compress start ========================================================================================\n'
        ...
        row20 = 'select segment_name,segment_type,blocks,round(BYTES / 1024 / 1024 / 1024,2) "Actual_Size(GB)" FROM dba_segments WHERE segment_name=' \
                + "'" + self.tabname + "'" + ';\n'
        row21 = 'select OWNER, TABLE_NAME, TABLESPACE_NAME, COMPRESSION, COMPRESS_FOR, DEGREE from dba_tables where table_name = ' + "'" + self.tabname + "';\n"
        row22 = 'select OWNER,TABLE_NAME,INDEX_NAME,STATUS,TABLESPACE_NAME,DEGREE from dba_indexes where table_name= ' + "'" + self.tabname + "';\n"
        row23 = 'alter table ' + self.user + '.' + self.tabname + ' move compress for oltp parallel 16;\n'
        ...
        ;row_list.append(row19);row_list.append(row20);row_list.append(row21);row_list.append(row22);row_list.append(row23)
    ...
    def third(self,file_name,dt1):
        ...
        row6 = 'select owner,table_name,blocks,empty_blocks,num_rows,avg_row_len,round(num_rows*avg_row_len/1024/1024/1024) "Estimate_Size(GB)",last_analyzed from dba_tables where table_name=' \
                + "'" + self.tabname + "'" + ';\n'
        row7 = 'select OWNER, TABLE_NAME, TABLESPACE_NAME, COMPRESSION, COMPRESS_FOR, DEGREE from dba_tables where table_name = ' + "'" + self.tabname + "';\n"
        row8 = 'select OWNER,TABLE_NAME,INDEX_NAME,STATUS,TABLESPACE_NAME,DEGREE from dba_indexes where table_name= ' + "'" + self.tabname + "';\n"

        row9 = 'select sysdate from dual;\n'
        row10 = 'prompt ' + dt1 + '_compress_' + self.tabname + ' Table_Compress end =======================================================================================\n'
        row11 = 'spool off;\n'
        row12 = 'exit\n'
        row_list = []
        row_list.append(row1);row_list.append(row2);row_list.append(row3);row_list.append(row4);row_list.append(row5);row_list.append(row6) \
